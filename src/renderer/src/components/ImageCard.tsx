...
import {
  FileProcess,
  FileProcessStatus,
} from "../interfaces/file-process.interface";
    ...
    <div>
      ...
      >
        <Stack spacing={1} direction="row">
          <img width={50} height={50} src={URL.createObjectURL(file.status === FileProcessStatus.processed ? file.optimizedFile : file.file)} />
          <Stack>
            <Typography variant="body2" className="text-white">
              {file.file.path}
            </Typography>
            <Typography variant="body2" className="text-white">
              {formatBytes(file.file.size)} {file.optimizedSize && `"-${formatBytes(file.file.size - file.optimizedSize)}"`} 
            </Typography>
          </Stack>
        </Stack>
